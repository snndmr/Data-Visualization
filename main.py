...
COLORS = ['#FF6666', '#AAA', '#000']


def draw_background(y, init, end):
    pyplot.hlines(y, init, end, colors=COLORS[0 if y == 1 else 1], lw=40)


def draw_detected(y, init, end):
    pyplot.hlines(y, init, end + 1, colors=COLORS[2], lw=40)


def draw_line(y, intervals):
    draw_background(y, 0, len(excel_data))
...
def get_ai_intervals(ai_list):
    ...
    for order, data in enumerate(ai_list):
        ...
        if order == len(ai_list) - 1 and counter > 0:
            intervals[index].append([order - counter + 1, order])
    return intervals
